defmodule MBTAV3API.Stops.RouteStopTest do
  ...
  describe "list_from_route_patterns/2" do
    test "Orange line (1 branch), direction 0" do
      with_mock(RoutesRepo, by_stop: fn _stop_id, _opts -> [] end) do
        with_mock(Repo,
          get: fn stop_id -> %Stop{id: stop_id} end,
          get_parent: fn _ -> %Stop{id: "parent"} end,
          stop_features: fn _, _ -> [] end
        ) do
          ...
          actual = RouteStop.list_from_route_patterns(route_pattern_with_stops, @orange_route, 0)
    ...
    test "Orange line (1 branch), direction 1" do
      with_mock(RoutesRepo, by_stop: fn _stop_id, _opts -> [] end) do
        with_mock(Repo,
          ...
          actual = RouteStop.list_from_route_patterns(route_pattern_with_stops, @orange_route, 1)
    ...
    test "Red line (2 branches), direction 0" do
      with_mock(RoutesRepo, by_stop: fn _stop_id, _opts -> [] end) do
        with_mock(Repo,
          ...
          actual = RouteStop.list_from_route_patterns(route_patterns_with_stops, @red_route, 0)
    ...
    test "Red line (2 branches), direction 1" do
      with_mock(RoutesRepo, by_stop: fn _stop_id, _opts -> [] end) do
        with_mock(Repo,
          ...
          actual = RouteStop.list_from_route_patterns(route_patterns_with_stops, @red_route, 1)
    ...
    test "Green (4 branches), direction 0" do
      with_mock(RoutesRepo, by_stop: fn _stop_id, _opts -> [] end) do
        with_mock(Repo,
          ...
          actual =
            RouteStop.list_from_route_patterns(route_patterns_with_stops, @green_route, 0, true)
    ...
    test "CR-Newburyport, direction 0" do
      with_mock(RoutesRepo, by_stop: fn _stop_id, _opts -> [] end) do
        with_mock(Repo,
          ...
          actual =
            RouteStop.list_from_route_patterns(route_patterns_with_stops, @newburyport_route, 0)
    ...
    test "CR-Newburyport, direction 1" do
      with_mock(RoutesRepo, by_stop: fn _stop_id, _opts -> [] end) do
        with_mock(Repo,
          ...
          actual =
            RouteStop.list_from_route_patterns(route_patterns_with_stops, @newburyport_route, 1)
    ...
    test "Hingham/Hull Ferry, direction 0" do
      with_mock(RoutesRepo, by_stop: fn _stop_id, _opts -> [] end) do
        with_mock(Repo,
          ...
          actual = RouteStop.list_from_route_patterns(route_patterns_with_stops, route, 1)
    ...
    test "handles an empty list of route patterns" do
      assert RouteStop.list_from_route_patterns([], @red_route, 0) == []
    end
    ...
    test "optionally uses the route ID for the branch name" do
      with_mock(RoutesRepo, by_stop: fn _stop_id, _opts -> [] end) do
        with_mock(Repo,
          ...
          assert_branch_names(
            RouteStop.list_from_route_patterns(
              route_pattern_with_stops_0,
              @green_b_route,
              0,
              true
            ),
  ...
  describe "list_from_shapes/4" do
    test "handles Red line when Ashmont/Braintree are first" do
      with_mock(Repo, get_parent: fn stop_id -> %Stop{id: stop_id} end) do
        ashmont_shape = %Shape{
          id: "ashmont",
          name: "Ashmont",
        ...
        actual =
          RouteStop.list_from_shapes([ashmont_shape, braintree_shape], stops, @red_route, 0)
    ...
    test "handles Red line when Ashmont/Braintree are last" do
      with_mock(Repo, get_parent: fn stop_id -> %Stop{id: stop_id} end) do
        ...
        actual =
          RouteStop.list_from_shapes([ashmont_shape, braintree_shape], stops, @red_route, 1)
    ...
    test "handles ferry routes with multiple shapes by returning the stops as-is" do
      with_mock(Repo, get_parent: fn stop_id -> %Stop{id: stop_id} end) do
        ...
        actual = RouteStop.list_from_shapes([primary, other], stops, route, 1)
  ...
  describe "build_route_stop/3" do
    test "creates a RouteStop object with all expected attributes" do
      result =
        RouteStop.build_route_stop(@stop, @red_route,
          first?: true,
          last?: true,
          branch: "Braintree"
        )
  ...
  describe "fetch_zone/1" do
    test "returns a RouteStop with the zone data" do
      with_mock(Repo, get: fn stop_id -> %Stop{id: stop_id, zone: "1"} end) do
        route_stop = RouteStop.build_route_stop(@stop, @red_route)
        fetched = RouteStop.fetch_zone(route_stop)
        assert fetched.zone
    ...
    test "returns a RouteStop when no zone data" do
      with_mock(Repo, get: fn stop_id -> %Stop{id: stop_id} end) do
        route_stop =
          RouteStop.build_route_stop(%Stop{name: "Alewife", id: "place-alfcl"}, @red_route)
  ...
  describe "fetch_stop_features/1" do
    test "returns a RouteStop with the stop feature data" do
      with_mock(Repo, stop_features: fn _, _ -> [] end) do
        route_stop = RouteStop.build_route_stop(@stop, @red_route)
        fetched = RouteStop.fetch_stop_features(route_stop)
  ...
  describe "fetch_connections/1" do
    test "builds a list of connecting routes at a stop" do
      ...
      with_mock(RoutesRepo,
        ...
        with_mock(Repo, get_parent: fn stop_id -> %Stop{id: stop_id} end) do
          fetched = RouteStop.fetch_connections(route_stop)
  ...
  describe "RouteStop implements Util.Position" do
    ...
    test "Position.Latitude" do
      assert Position.latitude(@red_route_stop) == 100.0
      assert Position.longitude(@red_route_stop) == 50.0
    end
  end
