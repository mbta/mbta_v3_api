defmodule MBTAV3API.Schedules.RepoTest do
  @moduledoc false
  ...
  alias MBTAV3API.Schedules.{HoursOfOperation, Repo, Schedule, Trip}
  ...
  describe "by_route_ids/2" do
    test "can take a route/direction/sequence/date" do
      with_mocks([
        {Schedules, [],
         all: fn _opts ->
           %JsonApi{
             data: [build(:schedule_data)]
           }
         end},
        {RoutesRepo, [], get: fn route_id -> %JsonApi{data: [%Item{id: route_id}]} end},
        {StopsRepo, [], get_parent: fn _ -> %JsonApi{data: [%Item{}]} end}
      ]) do
        assert [%Schedule{} | _] =
                 Repo.by_route_ids(["CR-Lowell"],
                   date: UtilDate.service_date(),
                   direction_id: 1,
    ...
    test "can take multiple route IDs" do
      with_mocks([
        {Schedules, [], all: fn _opts -> %JsonApi{data: [build(:schedule_data)]} end},
        {RoutesRepo, [], get: fn route_id -> %JsonApi{data: [%Item{id: route_id}]} end},
        {StopsRepo, [], get_parent: fn _ -> %JsonApi{data: [%Item{}]} end}
      ]) do
        assert [%Schedule{} | _] =
  ...
  describe "schedule_for_trip/2" do
    ...
    #          |> Repo.schedules_for_stop(direction_id: 1)
    #          |> List.first()
    #          |> Map.get(:trip)
    #          |> Map.get(:id)
  ...
  describe "trip/1" do
    ...
    #   date = Timex.shift(UtilDate.service_date(), days: 1)
  ...
  describe "end_of_rating/1" do
    test "returns the date if it comes back from the API" do
      ...
      assert ~D[2017-01-01] = Repo.end_of_rating(fn _ -> {:error, [error]} end)
    end

    test "returns nil if there are problems" do
      refute Repo.end_of_rating(fn _ -> %JsonApi{} end)
    end
  end
  ...
  describe "rating_dates/1" do
    test "returns the start/end dates if it comes back from the API" do
      ...
      assert {~D[2016-12-01], ~D[2017-01-01]} = Repo.rating_dates(fn _ -> {:error, [error]} end)
    end

    test "returns :error if there are problems" do
      assert Repo.rating_dates(fn _ -> %JsonApi{} end) == :error
    end
  end

  describe "hours_of_operation/2" do
    test "returns an %HoursOfOperation{} struct for a valid route" do
      with_mock(HoursOfOperation, hours_of_operation: fn _, _, _ -> %HoursOfOperation{} end) do
        assert %HoursOfOperation{} = Repo.hours_of_operation("47", Date.utc_today(), :desc)
      end
    end

    test "returns an %HoursOfOperation{} struct for an invalid route" do
      with_mock(HoursOfOperation, hours_of_operation: fn _, _, _ -> {:error, "bad response"} end) do
        assert %HoursOfOperation{} =
                 Repo.hours_of_operation("unknown route ID", Date.utc_today(), :desc)
      end
    end
  end
  ...
  describe "insert_trips_into_cache/1" do
    test "caches trips that were already fetched" do
      ...
      assert {:ok, %Trip{id: ^trip_id}} = ConCache.get(Repo, {:trip, trip_id})
    end
    ...
    test "caches trips that don't have the right data as nil" do
      ...
      data = [
        %Item{relationships: %{"trip" => [%Item{id: trip_id}]}}
      ]

      Repo.insert_trips_into_cache(data)
      assert ConCache.get(Repo, {:trip, trip_id}) == {:ok, nil}
    end
  end

  describe "valid?/1" do
    test "trips with an id are valid" do
      assert Repo.valid?(%Item{relationships: %{"trip" => [%Item{id: "1"}]}})
    end

    test "trips without an id are invalid" do
      refute Repo.valid?(%Item{relationships: %{"trip" => []}})
    end
  end
