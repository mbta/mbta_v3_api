defmodule MBTAV3API.Routes.PopulateCachesTest do
  ...
  describe "init/1" do
    test "sends :populate_all message" do
      _ = PopulateCaches.init(FakeRepo)
      assert_received :populate_all
    end

    test "stores the repo as its argument" do
      assert {:ok, FakeRepo} = PopulateCaches.init(FakeRepo)
    end
  end

  describe "handle_info/2" do
    test "populate_all: gets headsigns and shapes for each route" do
      assert {:noreply, FakeRepo} = PopulateCaches.handle_info(:populate_all, FakeRepo)
    ...
    test "ignores unknown messages" do
      assert {:noreply, FakeRepo} = PopulateCaches.handle_info(:unknown, FakeRepo)
    end
  end
end
