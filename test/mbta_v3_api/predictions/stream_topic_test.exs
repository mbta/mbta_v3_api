defmodule MBTAV3API.Predictions.StreamTopicTest do
  @moduledoc false

  use ExUnit.Case, async: false

  import MBTAV3API.Predictions.StreamTopic
  import Mock

  alias MBTAV3API.Predictions.StreamTopic
  alias MBTAV3API.RoutePatterns.Repo, as: RoutePatternsRepo
  alias MBTAV3API.RoutePatterns.RoutePattern

  setup_with_mocks([
    {RoutePatternsRepo, [], [by_stop_id: fn id -> mock_route_patterns(id) end]}
  ]) do
    :ok
  end
  ...
  describe "new/1" do
    test "works for stop id with route patterns" do
      ...
      assert [
               {[route: "Route1", direction: 0], "filter[route]=Route1&filter[direction_id]=0"}
               | _
             ] = streams
    end
