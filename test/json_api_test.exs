defmodule JsonApiTest do
  ...
  describe "parse/1" do
    test "parses an error into a JsonApi.Error struct" do
      body = """
      {
        "jsonapi": {"version": "1.0"},
        "errors": [
    ...
    test "parses invalid JSON into an error tuple" do
    ...
    test "parses a body into a JsonApi struct" do
      body = """
    ...
    test "parses a relationship that's present in data" do
      body = """
    ...
    test "handles a non-included relationship" do
      body = """
    ...
    @tag timeout: 5000
    test "handles a cyclical included relationship with properties" do
      {:ok, body} =
        Jason.encode(%{
          data: %{
            attributes: %{},
    ...
    test "handles an empty relationship" do
      body = """
    ...
    test "handles ServerSentEventStream data format" do
      list = ~s([{"type":"stop","links":{},"id":"20761","attributes":{}}])

      assert JsonApi.parse(list) == %JsonApi{
               data: [
                 %JsonApi.Item{id: "20761", type: "stop", attributes: %{}, relationships: %{}}
               ]
             }
