defmodule MBTAV3API.Predictions.StreamTopic do
  @moduledoc """
  ...
  alias MBTAV3API.Predictions.{Store, StreamSupervisor}
  ...
  defp streams_from_fetch_keys(stop: stop_id) do
    RoutePatternsRepo.by_stop_id(stop_id)
    |> Enum.map(&{to_keys(&1), to_filter_name(&1)})
  end

  defp to_keys(%RoutePattern{route_id: route_id, direction_id: direction_id}) do
    [route: route_id, direction: direction_id]
  end

  defp to_filter_name(%RoutePattern{route_id: route_id, direction_id: direction_id}) do
    %{
      route: route_id,
