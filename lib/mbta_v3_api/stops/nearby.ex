defmodule MBTAV3API.Stops.Nearby do
  ...
  defmodule Options do
    ...
    alias MBTAV3API.Routes.Repo, as: RoutesRepo
    alias MBTAV3API.Stops.{Nearby, Repo}

    defstruct api_fn: &Nearby.api_around/2,
              keys_fn: &Nearby.keys/1,
              fetch_fn: &Repo.get_parent/1,
              routes_fn: &RoutesRepo.by_stop_and_direction/2,
              limit: nil
  end
  ...
  * Return Subway stops in 5 mi radius
  ...
  defp api_task(position, %{api_fn: api_fn}, opts) do
  ...
  def keys(%{id: id}) do
    ...
    |> Enum.flat_map(fn direction_id ->
      id
      |> RoutesRepo.by_stop(direction_id: direction_id)
  ...
  iex> MBTAV3API.Stops.Nearby.no_more_than([1, 2, 3, 4, 5], 2, fn i -> [rem(i, 2)] end)
  [1, 2, 3, 4]

  iex> MBTAV3API.Stops.Nearby.no_more_than([1, 2, 3, 4, 5], 2, fn i -> [rem(i, 2), div(i, 2)] end)
  [1, 2, 4, 5]

  iex> MBTAV3API.Stops.Nearby.no_more_than([1, 2, 3, 4, 5], 1, fn i -> [rem(i, 2)] end)
  [1, 2]
  """
