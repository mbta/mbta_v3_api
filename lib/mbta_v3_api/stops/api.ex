defmodule MBTAV3API.Stops.Api do
  @moduledoc """
  ...
  def by_gtfs_id(gtfs_id, opts \\ []) do
    stops_by_gtfs_id_fn = Keyword.get(opts, :stops_by_gtfs_id_fn, &Stops.by_gtfs_id/2)

    gtfs_id
    |> stops_by_gtfs_id_fn.(@default_params)
    |> extract_v3_response()
    |> parse_v3_response()
  end
  ...
  def by_trip(trip_id, opts \\ []) do
    trips_by_id_fn = Keyword.get(opts, :trips_by_id_fn, &Trips.by_id/2)

    case trips_by_id_fn.(trip_id, include: "stops") do
      %JsonApi{
        data: [
          %JsonApi.Item{
            relationships: %{
  ...
  @spec v3_parking(Item.t()) :: [ParkingLot.t()]
  ...
  @spec parse_parking_area(Item.t()) :: ParkingLot.t()
  ...
  @spec to_parking_lot(map) :: ParkingLot.t()
  defp to_parking_lot(props) do
    %ParkingLot{
      id: Map.get(props, "id"),
      name: Map.get(props, "name"),
      address: Map.get(props, "address"),
      capacity: Helpers.struct_or_nil(Capacity.parse(props)),
      payment: Helpers.struct_or_nil(Payment.parse(props)),
      utilization: Helpers.struct_or_nil(Utilization.parse(props)),
      note: Map.get(props, "note"),
      manager: Helpers.struct_or_nil(Manager.parse(props)),
